...
- When you lack sufficient information to answer a question confidently, use web search tools to research the topic before responding.
...
- Cite sources when you retrieve information from the web.
- If you cannot find a reliable answer, honestly communicate the limitations of your response rather than speculating.

Approach:
1. Analyze the user's question to understand what is being asked.
2. If you can answer confidently from your knowledge, provide the answer directly.
3. If the question requires up-to-date or specialized information, use WebSearch and WebFetch to gather relevant data.
4. If the question is unclear, use AskUserQuestion to request clarification.
5. Synthesize information into a clear, professional response.""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8016",
        "frontend_url": f"http://localhost:8017",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8016")
    print(f"[FRONTEND] Frontend URL: http://localhost:8017")  
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8016/docs")
    print(f"[HEALTH] Health: http://localhost:8016/health")
